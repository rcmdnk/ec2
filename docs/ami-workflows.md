...
This guide explains how to prepare the `cli_input_json` files used by `ec2
launch` and `ec2 submit`. The files are generated by `ec2 setup` after an AMI
...
2. Create an EC2 key pair and set `EC2_KEY_NAME`.
3. Set `AWS_REGION` and the subnet settings in the environment file.
4. Enable the AMI families to generate, such as `CPU` or `GPU`.
5. Configure EFS, FSx, S3 Files, or io2 settings if the launched instances
   need shared storage.
6. Set the instance type for each family with

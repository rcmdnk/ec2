...
[[ "$AMI_PROGRESS_POLL_DELAY_SECONDS" =~ ^[1-9][0-9]*$ ]] || {
  echo 'AMI_PROGRESS_POLL_DELAY_SECONDS must be a positive integer.' >&2
  exit 1
}
AMI_PACKER_ON_ERROR=${AMI_PACKER_ON_ERROR-cleanup}
case "$AMI_PACKER_ON_ERROR" in
  cleanup|abort|ask) ;;
  *) echo 'AMI_PACKER_ON_ERROR must be cleanup, abort, or ask.' >&2; exit 1 ;;
esac

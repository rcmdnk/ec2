...
REQUIRED_SETTINGS_EC2=(SUBNET_IDS KEY_NAME)
...
declare -p AWS_ARGS &>/dev/null || AWS_ARGS=(--profile "$PROFILE" --region "$REGION")
EC2_USER="${EC2_USER-ec2-user}"

AWS_POLL_DELAY_SECONDS=${AWS_POLL_DELAY_SECONDS-10}
AWS_MAX_ATTEMPTS=${AWS_MAX_ATTEMPTS-400}
FS_MOUNT_MAX_ATTEMPTS=${FS_MOUNT_MAX_ATTEMPTS-60}
FS_MOUNT_RETRY_INTERVAL=${FS_MOUNT_RETRY_INTERVAL-2}
PACKER_VALIDATE_ONLY=${PACKER_VALIDATE_ONLY-0}

SSH_INTERFACE=${SSH_INTERFACE-private_ip}
SSH_USERNAME=${SSH_USERNAME-${EC2_USER}}
...
DEVICE_NAME=${DEVICE_NAME-/dev/xvda}
...
SHUTDOWN=${SHUTDOWN-systemd}
SHUTDOWN_DURATION=${SHUTDOWN_DURATION-hourly}
SHUTDOWN_CHECK_PROCESS=${SHUTDOWN_CHECK_PROCESS-sshd|screen|tmux|mosh-server}
PACKAGES=${PACKAGES-}
FLATPAK_PACKAGES=${FLATPAK_PACKAGES-}
UPDATE_PACKAGES=${UPDATE_PACKAGES-1}
MULTI_PROCESSING=${MULTI_PROCESSING-1}

# CPU instance configuration.
CPU_INSTANCE=${CPU_INSTANCE-1}
CPU_AMI_NAME=${CPU_AMI_NAME-}
CPU_AMI_FILTER=${CPU_AMI_FILTER-al2023-ami-*-x86_64} # Amazon Linux 2023
CPU_AMI_OWNER=${CPU_AMI_OWNER=amazon}
CPU_SOURCE_AMI_ID=${CPU_SOURCE_AMI_ID-}
CPU_INSTANCE_TYPE=${CPU_INSTANCE_TYPE-c8i.2xlarge}
CPU_ADDITIONAL_PACKAGES=${CPU_ADDITIONAL_PACKAGES-}

# GPU instance configuration.
GPU_INSTANCE=${GPU_INSTANCE-0}
GPU_AMI_NAME=${GPU_AMI_NAME-}
GPU_AMI_FILTER="${GPU_AMI_FILTER-Deep Learning OSS Nvidia Driver AMI GPU PyTorch * (Amazon Linux 2023)*}" # suffix contains release date
GPU_AMI_OWNER=${GPU_AMI_OWNER-amazon}
GPU_SOURCE_AMI_ID=${GPU_SOURCE_AMI_ID-}
GPU_INSTANCE_TYPE=${GPU_INSTANCE_TYPE-"g4dn.xlarge"}
GPU_ADDITIONAL_PACKAGES=${GPU_ADDITIONAL_PACKAGES-}

# An enabled family needs a name: Packer builds under it and setup_ec2 looks the
# image up by it.
for family in CPU GPU; do
  enabled_var="${family}_INSTANCE" name_var="${family}_AMI_NAME"
...
FSX_STORAGE_CAPACITIES=${FSX_STORAGE_CAPACITIES-100}
FSX_THROUGHPUT_CAPACITIES=${FSX_THROUGHPUT_CAPACITIES-160}
...
IO2_SIZES=${IO2_SIZES-100}
IO2_IOPSS=${IO2_IOPSS-3000}
IO2_DEVICES=${IO2_DEVICES-/dev/sdf}
...
EC2_SSH_USER=${EC2_SSH_USER-${EC2_USER}}
EC2_PUBLIC_IP=${EC2_PUBLIC_IP-false}
EC2_PRIVATE_IP=${EC2_PRIVATE_IP-1}
EC2_INSTANCE_TYPE=${EC2_INSTANCE_TYPE-t3.medium}
EC2_SPOT_INSTANCE=${EC2_SPOT_INSTANCE-0}
...
EC2_USER_DATA=${EC2_USER_DATA-$ec2_user_data_default}
unset ec2_user_data_default
EC2_MOSH_SERVER=${EC2_MOSH_SERVER-/usr/bin/mosh-server}

FS_DIR=${FS_DIR-}
FS_USR=${FS_USR-1}
FS_OPT_SCRIPTS=${FS_OPT_SCRIPTS-}
FS_DOTFILES_FILE=${FS_DOTFILES_FILE-}
FS_DOTFILES_DIR=${FS_DOTFILES_DIR-}
FS_DOTFILES_CONFIG_FILE=${FS_DOTFILES_CONFIG_FILE-}
FS_DOTFILES_CONFIG_DIR=${FS_DOTFILES_CONFIG_DIR-}
